...
class _DropdownMenuPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final bool buildToBottom = menuLimits?.buildToBottom ?? true;

    final Tween<double> top = Tween<double>(
      begin: buildToBottom ? 1 : size.height,
      end: buildToBottom ? 0 : 0,
    );

    final Tween<double> bottom = Tween<double>(
      begin: buildToBottom ? 0 : size.height,
      end: buildToBottom ? size.height : size.height,
    );
...
class _DropdownMenuItemButtonState<T>
  ...
  Widget build(BuildContext context) {
    ...
    bool buildToBottom = menuLimits?.buildToBottom ?? true;
    if ((widget.itemIndex == 0 && buildToBottom) ||
        (widget.itemIndex == widget.route.items.length - 1 && !buildToBottom)) {
      /// first visible item
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  void initState() {
    ...
    _fadeOpacity = CurvedAnimation(
      ...
      curve: const Interval(0.4, 0.5), //originally it was 0 to 0.25
      reverseCurve: const Interval(0.75, 1.0),
    );
    _resize = CurvedAnimation(
      parent: widget.route.animation,
      curve: const Interval(0.25, 0.7), //originally it was 0.25 to 0.5
...
class _DropdownMenuRouteLayout<T> extends SingleChildLayoutDelegate {
  ...
  BoxConstraints getConstraintsForChild(BoxConstraints constraints) {
    ...
    final double maxHeight = math.max(0.0, 5 * _kMenuItemHeight + 16);
...
//  }
  ...
  ///override this to be shown precisely below / above the button
  _MenuLimits getMenuLimits(
      Rect buttonRect, double availableHeight, int index) {
    final double maxMenuHeight = 5 * _kMenuItemHeight + 16;
    double preferredMenuHeight = kMaterialListPadding.vertical;
    if (items.isNotEmpty)
      preferredMenuHeight +=
          itemHeights.reduce((double total, double height) => total + height);
    ...
    double menuTop = buttonRect.bottom;
    double menuBottom = menuTop + menuHeight;
    bool isBuildToBottom = true;

    if (menuBottom > availableHeight) {
      menuBottom = availableHeight;
    }

    if (menuBottom - menuTop < menuHeight) {
      if (buttonRect.top > menuHeight) {
        menuBottom = buttonRect.top;
        menuTop = menuBottom - menuHeight;
        isBuildToBottom = false;
      }
    }
    ...
    return _MenuLimits(
        menuTop, menuBottom, menuHeight, scrollOffset, isBuildToBottom);
  }
}
...
class _MenuItem<T> extends SingleChildRenderObjectWidget {
  ...
  final AdvDropdownMenuItem<T> item;
...
/// An item in a menu created by a [AdvDropdownButton].
///
/// The type `T` is the type of the value the entry represents. All the entries
/// in a given menu must represent values with consistent types.
class AdvDropdownMenuItem<T> extends _DropdownMenuItemContainer {
  /// Creates an item for a dropdown menu.
  ///
  /// The [child] argument is required.
  const AdvDropdownMenuItem({
    Key key,
    this.value,
  ...
  ///
  /// Eventually returned in a call to [AdvDropdownButton.onChanged].
  final T value;
}

/// An inherited widget that causes any descendant [AdvDropdownButton]
/// widgets to not include their regular underline.
///
/// This is used by [DataTable] to remove the underline from any
/// [AdvDropdownButton] widgets placed within material data tables, as
...
class DropdownButtonHideUnderline extends InheritedWidget {
  ...
  /// Returns whether the underline of [AdvDropdownButton] widgets should
  /// be hidden.
...
/// Typically, an enum is used. Each [AdvDropdownMenuItem] in [items] must be
...
/// See also:
///
///  * [AdvDropdownMenuItem], the class used to represent the [items].
///  * [DropdownButtonHideUnderline], which prevents its descendant dropdown buttons
///    from displaying their underlines.
///  * [RaisedButton], [FlatButton], ordinary buttons that trigger a single action.
///  * <https://material.io/design/components/menus.html#dropdown-menu>
class AdvDropdownButton<T> extends StatefulWidget {
  ...
  /// be displayed.
  final List<AdvDropdownMenuItem<T>> items;

  /// The value of the currently selected [AdvDropdownMenuItem].
  ///
  ...
  /// [AdvDropdownMenuItem]s in [items].
  ///
  /// When a [AdvDropdownMenuItem] is selected, the widget that will be displayed
  /// from the list corresponds to the [AdvDropdownMenuItem] of the same index
  /// in [items].
  ///
  /// {@tool snippet --template=stateful_widget_scaffold}
  ///
  /// This sample shows a `DropdownButton` with a button with [Text] that
  /// corresponds to but is unique from [AdvDropdownMenuItem].
  ///
  /// ```dart
  ...
  /// {@end-tool}
  ///
  /// If this callback is null, the [AdvDropdownMenuItem] from [items]
  ...
  @override
  _AdvDropdownButtonState<T> createState() => _AdvDropdownButtonState<T>();
}

class _AdvDropdownButtonState<T> extends State<AdvDropdownButton<T>>
  ...
  void initState() {
    ...
    if (widget.outerActions != null) {
      widget.outerActions.forceTap = _handleTap;
    }
  ...
  @override
  void didUpdateWidget(AdvDropdownButton<T> oldWidget) {
  ...
  void _updateSelectedIndex() {
    ...
    assert(widget.value == null ||
        widget.items
                .where(
                    (AdvDropdownMenuItem<T> item) => item.value == widget.value)
                .length ==
            1);
    _selectedIndex = null;
  ...
  void _handleTap() {
    ...
    if (widget.outerActions.onTap != null) widget.outerActions.onTap();
    ...
    /// set the margin to be zero so the pop up will show precisely like the button's width
      ...
      child: Actions(
        ...
        child: Focus(
          ...
          child: GestureDetector(
            ...
            child: Container(padding: widget.padding, child: result),
          ),
        ),
      ),
    );
...
/// A convenience widget that wraps a [AdvDropdownButton] in a [FormField].
class DropdownButtonFormField<T> extends FormField<T> {
  /// Creates a [AdvDropdownButton] widget wrapped in an [InputDecorator] and
  /// [FormField].
  ///
  ...
  DropdownButtonFormField({
    Key key,
    T value,
    @required List<AdvDropdownMenuItem<T>> items,
